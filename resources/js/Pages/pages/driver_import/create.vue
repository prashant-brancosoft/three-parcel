  ...
  setup(props) {
    ...
    const handleSubmit = async () => {
      ...
      if (Object.keys(errors.value).length > 0) {
        ...
        let response;
        console.log("DriverImported",props.DriverImported);
        if (props.DriverImported) {
          response = await axios.post(`/driver-import/reupload-invalid-file-store/${props.DriverImported.id}`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          });
        } else {
          response = await axios.post(`/driver-import/import-file`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          });
        ...
        if (response.status === 201) {
           Swal.fire({
             title: 'Success!',
             text: t('driver_files_imported_successfully'),
             icon: 'success',
           }).then(async (result) => {
                if (result.isConfirmed) {
                  form.reset();
                  router.get('/driver-import');
                }
            });
...
<template>
  <Layout>
    ...
    <PageHeader :title="bannerimage ? $t('edit') : $t('create')" :pageTitle="$t('driver-import')" pageLink="/driver-import"/>
    <BRow>
      <BCol lg="12">
            ...
            <form @submit.prevent="handleSubmit">
              <FormValidation :form="form" :rules="validationRules" ref="validationRef">
                <div class="row">
                  <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="firebase_json" class="form-label">{{$t("Driver Files")}}
                  ...
                  <div class="col-lg-12">
                    <div class="text-end">
                      <button type="submit" class="btn btn-primary"> {{ DriverImported ? $t('update') : $t('save') }}</button>
                    </div>
                  </div>
                </div>                 
