...
export default {
    ...
    setup(props) {
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/user-import/list`, { params });
    ...
    {
        router.get(`/user-import/reupload-invalid-file/${result}`); 
    }

    const downloadInvalid = async (result) =>{
        const response = await axios.get(
        `/user-import/download-invalid-file/${result}`,
        ...
        link.href = url;
        link.setAttribute('download', 'invalid_users.xlsx'); //  set filename
    ...
    const sampleFileDownload = async() => {
        ...
        const response = await axios.get('/user-import/sample-download', {
            responseType: 'blob'
        });
        ...
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'sample_upload.xlsx'); //  set filename
        document.body.appendChild(link);
        link.click();
...
<template>
    <Layout>
        ...
        <PageHeader :title="$t('user-import')" :pageTitle="$t('user-import')" />
        <BRow>
            <BCol lg="12">
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">    
                                    ...
                                    <Link href="/user-import/create" v-if="permissions.includes('upload_bulk_file')">
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <BButton   @click.prevent="downloadInvalid(result.id)" v-if="permissions.includes('download_bulk_file') && result.invalid_file == 1"
                                                   ...
                                                   <BButton  @click.prevent="reuploadInvalid(result.id)"  v-if="permissions.includes('reupload_bulk_file') && result.invalid_file == 1"
